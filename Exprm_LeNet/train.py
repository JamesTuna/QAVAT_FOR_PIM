...
model = LeNet5(nbits_activation=nbits_activation,nbits_weight=nbits_weight)
...
# data prep
train_loader,test_loader = get_loader('MNIST',batch_size=128, test_size=10000,test_batch_size=1000)
...
train(model,train_loader,test_loader,config,imgSize=28,imgFlat=False,lossfunc=lossfunc,printPerEpoch=1)

# save
torch.save(model.state_dict(),"saved/LeNet_A%sW%s_noise%s.ckpt"%(nbits_activation,nbits_weight,config['noise_std']))
