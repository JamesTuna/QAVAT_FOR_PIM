...
model = LeNet5(nbits_weight=args.nbits_weight,nbits_activation=args.nbits_act)


_,test_loader = get_loader('MNIST',batch_size=100, test_size=args.testSize,test_batch_size=1000)
...
result = test(test_loader,model,noise_std=args.noise,
                      repeat=args.testSample,device=device,
                                    imgSize=28,imgFlat=False,debug=True,lossfunc=torch.nn.CrossEntropyLoss())
